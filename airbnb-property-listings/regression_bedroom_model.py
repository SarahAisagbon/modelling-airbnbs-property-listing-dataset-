...
hyper_param_dict = {
    ...
    "DecisionTreeRegressor": {
        ...
        "min_samples_split" : np.arange(0.2, 1, 0.2), 
        "max_features" : [6, 7, 8, 9] 
    }
}

models_list = hyper_param_dict.keys()
...
def evaluate_all_models(task_folder="models/regression"):
    ...
    stochastic_gradient_descent_model = tune_regression_model_hyperparameters("SGDRegressor", X_train, y_train, X_validation, y_validation, X_test, y_test, hyperpara_searchspace = hyper_param_dict["SGDRegressor"])
    ...
    decision_tree_model = tune_regression_model_hyperparameters("DecisionTreeRegressor", X_train, y_train, X_validation, y_validation, X_test, y_test, hyperpara_searchspace = hyper_param_dict["DecisionTreeRegressor"])
    ...
    random_forest_model = tune_regression_model_hyperparameters("RandomForestRegressor", X_train, y_train, X_validation, y_validation, X_test, y_test, hyperpara_searchspace = hyper_param_dict["RandomForestRegressor"])
    ...
    gradient_boosting_model = tune_regression_model_hyperparameters("GradientBoostingRegressor", X_train, y_train, X_validation, y_validation, X_test, y_test, hyperpara_searchspace = hyper_param_dict["GradientBoostingRegressor"])
    ...
    return stochastic_gradient_descent_model, decision_tree_model, random_forest_model, gradient_boosting_model
...
if __name__ == "__main__":
    ...
    category_row = clean_data["Category"]
    one_hot = pd.get_dummies(category_row)
    X, y = tabular_data.load_airbnb(clean_data, "beds")
