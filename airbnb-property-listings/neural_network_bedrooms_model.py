...
class  AirbnbNightlyPriceImageDataset(Dataset):
    def __init__(self) -> None:
        ...
        category_row = data["Category"]
        one_hot = pd.get_dummies(category_row)
        self.X, self.y = tabular_data.load_airbnb(data, "beds")
...
class NNModel(Module):
    def __init__(self, config) -> None:
        ...
        depth = config["depth"]
        layers = []
        layers.append(torch.nn.Linear(15, width))
...
def save_model(model, hyper_dict, nn_metrics, folder="models/regression_bedroom/neural_networks/"):
...
def find_best_nn(epochs):
    ...
    save_model(best_model, best_hyper_dict, best_nn_metrics, "models/regression_bedroom/neural_networks/best_neural_networks")
